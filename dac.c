...
uint32_t Data = 0;
float Data1 = 0; //dit is nu een float omdat de deling in de dac functie dan mooier gaat. zo kan er toch een kleinere frequentiestap worden gemaakt dan 2 Hz
//uint32_t Data2 = 250;

float freq = 1000; //de frequentie start met 1000 Hz
 ...
 */
{
	...
	TIM_TimeBaseStructure.TIM_Period = 209; //dit is het aantal ticks voor de timer over loopt
											 //als de timer over loopt word de interrupt getriggerd
											 //ik wil samples van 200 khz omdat dat ruim boven het
											 //kantelpunt van het reconstructiefilter zit en net
											 //onder de maximale frequentie
											 //42000000 / 200000 - 1 = 209
 ...
 */
{
	if (TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET)
	{
		...
		DAC_SetChannel1Data(DAC_Align_12b_R, 100 + LUT[(int)Data1]);
